...
- No OAuth server / app-install flow. `shopli` never mints tokens; the user
...
- Provisioned by the user either via store admin → *Settings → Apps → Develop
  ...
  distribution app (one-time OAuth install mints an offline token). `shopli`
...
- **Credential resolution chain** (first found wins), mirroring `aws`/`gh`/`docker`:
  1. `--token` flag
  2. `SHOPLI_TOKEN` environment variable (+ `SHOPLI_SHOP` for the domain)
...
`shopli::run()`. The library split makes the logic testable.

Module tree mirrors the dependency stack (lower modules know nothing of higher):

```
src/
├── main.rs            # parse args → shopli::run() → print errors
...
│   ├── query.rs       # `shopli query`
│   ├── store.rs       # `shopli store add/list/use/remove`
│   ├── export.rs      # `shopli export`
│   ├── import.rs      # `shopli import`
│   └── clone.rs       # `shopli clone`  (export from A → import into B)
└── resource/
...
```
shopli store add <name> --shop <domain> [--token <tok>]
shopli store list
shopli store use <name>
shopli store remove <name>
shopli query <graphql> [--store <name>] [--json]
shopli export <resource> [--store <name>] [--out <file>]
shopli import <resource> --file <f> [--store <name>] [--dry-run]
shopli clone --from <store> --to <store> [--only products,discounts] [--dry-run]
```
...
- Location: `dirs::config_dir()/shopli/config.toml` (e.g. `~/.config/shopli/config.toml`).
- Shape:
  ```toml
  default = "acme-dev"
